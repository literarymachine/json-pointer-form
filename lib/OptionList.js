          ...
          {values.map((option, index) => (
            <li key={option}>
              <input
                ...
                onChange={(e) => {this.handleChange(e); this.setState({filter: ""})}}
                defaultChecked={true}
                value={option}
              />
              <label htmlFor={`${this.props.name}-${option}`} tabIndex="0">{option}</label>
            </li>
          ))}
        </ul>
                  ...
                  return (
                    <li key={option}>
                      <input
                        ...
                        id={`${this.props.name}-${option}`}
                        onChange={(e) => {this.handleChange(e); this.setState({filter: "", dropdown: false})}}
